...
function xorEncrypt(input: number[], password: number[]) {
  const encryptedArray = input.map((byte, index) =>
    bit32.bxor(byte, password[index % password.size()])
  );
...
function subscribe(name: string) {
  ...
  subscription = ms.SubscribeAsync("comradio:" + name, (message: unknown) => {
    ...
    const author: string = game
      .GetService("Players")
      .GetNameFromUserIdAsync(request.Author)!;
    const messagetype: string = request.Type;
    const tag: string = Tags(author);
    print(tag);
    if (messagetype === "text") {
      const content = text
        .FilterStringAsync(request.Content, owner.UserId)!
        .GetChatForUserAsync(owner.UserId);
      const box = output(tag + content);
    } else if (messagetype === "welcome") {
      const box = output(
        `Welcome, ${author}! Say "/rchelp" in the chat for a list of commands.`
      );
    } else if (messagetype === "status") {
      const comment = text
        .FilterStringAsync(request.Comment!, owner.UserId)!
        .GetChatForUserAsync(owner.UserId);
      ...
      if (target === owner.UserId) {
        switch (comment.split(";")[0]) {
          case "1a": // bob
            ...
            exchanges[request.Author].p = tonumber(
              request.Content.split(";")[0]
            )!;
            exchanges[request.Author].g = tonumber(
              request.Content.split(";")[1]
            )!;
            box.Destroy();
            const B =
              (exchanges[request.Author].g ^ privateKey) %
              exchanges[request.Author].p;
            send(
              request.Content + ";" + B,
              "diffieHellmanExchange",
              owner.UserId,
          ...
          case "1b": // alice
            ...
            exchanges[request.Author].B = tonumber(
              request.Content.split(";")[2]
            )!;
            box.Destroy();
            const A =
              (exchanges[request.Author].g ^ privateKey) %
          ...
          case "2a": // bob
            ...
            exchanges[request.Author].s =
              (exchanges[request.Author].A ^ privateKey) %
              exchanges[request.Author].p;
            send(
              "confirmed",
              "diffieHellmanExchange",
          ...
          case "2b": // alice
            exchanges[request.Author].s =
              (exchanges[request.Author].B ^ privateKey) %
              exchanges[request.Author].p;
            send(
              "confirmed",
              "diffieHellmanExchange",
          ...
          case "3": // bob
            box.Text +=
              "[KEYS CONFIFRMED] You can now send encrypted messages to " +
              author;
      ...
      switch (messagetype) {
        ...
        case "sound":
          ...
          break;
        case "ping":
          const pingTarget: Player = players.GetPlayerByUserId(
            tonumber(request.Content)!
          ) as Player;
          if (pingTarget) {
          ...
          if (messageTarget === owner.UserId) {
            ...
            if (keys[request.Author]) {
              box.Text =
                tag +
                "[ENCRYPTED] " +
                xorDecrypt(comment, encode(tostring(keys[request.Author])));
            } else {
              box.Text =
...
players.GetPlayers().forEach((player: Player) => {
  player.Chatted.Connect((command: string) => {
    if (command.sub(1, 6) === "/send ") {
      ...
      output(
        "/private [name] [msg] - send a private message to someone [requires keys]"
      );
      output("---------------------------------------------------");
      ...
      if (keys[id]) {
        const encrypted = xorEncrypt(
          encode(comment),
          encode(tostring(keys[id]))
        );
        send(tostring(id), "encrypted", player.UserId, encrypted);
      } else {
        output(
          "[ERROR]: Attempt to send private message to someone without encryption key"
        );
      }
    }
  });
