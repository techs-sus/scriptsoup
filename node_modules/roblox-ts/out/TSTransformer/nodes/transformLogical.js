...
function getLogicalChain(state, binaryExp, binaryOperatorKind, enableInlining) {
    return flattenByOperator(binaryExp, binaryOperatorKind).map((node, index, array) => {
        ...
        if (enableInlining) {
            ...
            inline = LuauAST_1.default.list.isEmpty(statements) && !willWrap;
        }
...
function buildInlineConditionExpression(state, node, tsBinaryOperator, luaBinaryOperator, buildCondition) {
    ...
    const conditionId = LuauAST_1.default.tempId("condition");
...
function transformLogical(state, node) {
    if (node.operatorToken.kind === typescript_1.default.SyntaxKind.AmpersandAmpersandToken) {
        ...
        return buildInlineConditionExpression(state, node, node.operatorToken.kind, "or", (conditionId, node) => LuauAST_1.default.unary("not", (0, createTruthinessChecks_1.createTruthinessChecks)(state, conditionId, node)));
    }
        ...
        const conditionId = LuauAST_1.default.tempId("condition");
        buildLogicalChainPrereqs(state, chain, conditionId, conditionId => LuauAST_1.default.binary(conditionId, "==", LuauAST_1.default.nil()));
        return conditionId;
    }
