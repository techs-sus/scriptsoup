...
function transformCaseClause(state, node, switchExpression, fallThroughFlagId, canFallThroughTo, shouldUpdateFallThroughFlag) {
    ...
    const statements = (0, transformStatementList_1.transformStatementList)(state, nonEmptyStatements.length === 1 && typescript_1.default.isBlock(firstStatement) ? firstStatement.statements : node.statements);
...
function transformSwitchStatement(state, node) {
    ...
    for (let i = 0; i < node.caseBlock.clauses.length; i++) {
        ...
        if (typescript_1.default.isCaseClause(caseClauseNode)) {
            ...
            LuauAST_1.default.list.pushList(statements, (0, transformStatementList_1.transformStatementList)(state, caseClauseNode.statements));
            break;
        }
    }
