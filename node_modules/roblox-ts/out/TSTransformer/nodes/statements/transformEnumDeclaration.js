...
function hasMultipleDefinitions(symbol) {
    ...
    for (const declaration of (_a = symbol.getDeclarations()) !== null && _a !== void 0 ? _a : []) {
        if (typescript_1.default.isEnumDeclaration(declaration) &&
...
function transformEnumDeclaration(state, node) {
    ...
    const id = (0, transformIdentifier_1.transformIdentifierDefined)(state, node.name);
    const statements = state.capturePrereqs(() => {
        ...
        for (const member of node.members) {
            (0, assert_1.assert)(typescript_1.default.isIdentifier(member.name) || typescript_1.default.isStringLiteral(member.name));
    ...
    const list = LuauAST_1.default.list.make(LuauAST_1.default.create(LuauAST_1.default.SyntaxKind.DoStatement, { statements }));
    if (symbol && state.isHoisted.get(symbol) !== true) {
        LuauAST_1.default.list.unshift(list, LuauAST_1.default.create(LuauAST_1.default.SyntaxKind.VariableDeclaration, { left: id, right: undefined }));
    }
    return list;
}
