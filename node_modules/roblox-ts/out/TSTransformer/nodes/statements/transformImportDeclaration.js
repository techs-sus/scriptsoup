...
function transformImportDeclaration(state, node) {
    ...
    if (importClause) {
        ...
        if (uses > 1) {
            ...
            const id = LuauAST_1.default.tempId((0, cleanModuleName_1.cleanModuleName)(moduleName[moduleName.length - 1]));
        ...
        if (importClause.name) {
            const symbol = state.getOriginalSymbol(importClause.name);
            if (state.resolver.isReferencedAliasDeclaration(importClause) && (!symbol || (0, isSymbolOfValue_1.isSymbolOfValue)(symbol))) {
                const moduleFile = (0, getSourceFileFromModuleSpecifier_1.getSourceFileFromModuleSpecifier)(state.typeChecker, node.moduleSpecifier);
                ...
                if (moduleSymbol && state.getModuleExports(moduleSymbol).some(v => v.name === "default")) {
                    LuauAST_1.default.list.pushList(statements, (0, transformVariableStatement_1.transformVariable)(state, importClause.name, LuauAST_1.default.property(importExp.get(), "default"))[1]);
                }
                else {
                    LuauAST_1.default.list.pushList(statements, (0, transformVariableStatement_1.transformVariable)(state, importClause.name, importExp.get())[1]);
                }
            }
        }
        if (importClause.namedBindings) {
            if (typescript_1.default.isNamespaceImport(importClause.namedBindings)) {
                LuauAST_1.default.list.pushList(statements, (0, transformVariableStatement_1.transformVariable)(state, importClause.namedBindings.name, importExp.get())[1]);
            }
            else {
                for (const element of importClause.namedBindings.elements) {
                    const symbol = state.getOriginalSymbol(element.name);
                    if (state.resolver.isReferencedAliasDeclaration(element) && (!symbol || (0, isSymbolOfValue_1.isSymbolOfValue)(symbol))) {
                        LuauAST_1.default.list.pushList(statements, (0, transformVariableStatement_1.transformVariable)(state, element.name, LuauAST_1.default.property(importExp.get(), ((_a = element.propertyName) !== null && _a !== void 0 ? _a : element.name).text))[1]);
                    }
                }
            }
        }
