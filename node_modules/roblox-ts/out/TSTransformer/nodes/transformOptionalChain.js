        ...
        if (isCompoundCall(item)) {
            ...
            if (item.callOptional) {
                if (item.kind === OptionalChainItemKind.PropertyCall) {
                    baseExpression = LuauAST_1.default.property((0, convertToIndexableExpression_1.convertToIndexableExpression)(baseExpression), item.name);
                }
                else {
                    baseExpression = LuauAST_1.default.create(LuauAST_1.default.SyntaxKind.ComputedIndexExpression, {
                        ...
                        index: (0, transformExpression_1.transformExpression)(state, item.argumentExpression),
                    });
                }
            }
        }
        const [result, prereqStatements] = state.capture(() => {
            ...
            const [newValue, ifStatements] = state.capture(() => {
                ...
                if (isCompoundCall(item) && item.callOptional) {
                    ...
                    newExpression = (0, wrapReturnIfLuaTuple_1.wrapReturnIfLuaTuple)(state, item.node, LuauAST_1.default.call(tempId, args));
                }
                else {
            ...
            const isUsed = !LuauAST_1.default.isNilLiteral(newValue) && !(0, isUsedAsStatement_1.isUsedAsStatement)(item.node);
            ...
            return isUsed ? tempId : LuauAST_1.default.nil();
        });
