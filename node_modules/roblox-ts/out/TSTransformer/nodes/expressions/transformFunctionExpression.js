...
function transformFunctionExpression(state, node) {
    ...
    if (typescript_1.default.isFunctionBody(body)) {
        LuauAST_1.default.list.pushList(statements, (0, transformStatementList_1.transformStatementList)(state, body.statements));
    }
    else {
        const [returnStatement, prereqs] = state.capture(() => (0, transformReturnStatement_1.transformReturnStatementInner)(state, body));
