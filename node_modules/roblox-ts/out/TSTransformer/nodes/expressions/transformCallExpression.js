...
function runCallMacro(macro, state, node, expression, nodeArguments) {
    ...
    if (!LuauAST_1.default.list.isEmpty(prereqs) && (0, expressionMightMutate_1.expressionMightMutate)(state, expression, nodeExpression)) {
...
function isInsideRoactComponent(state, node) {
    const classLikeAncestor = (0, traversal_1.getAncestor)(node, typescript_1.default.isClassLike);
    if (classLikeAncestor) {
        return (0, extendsRoactComponent_1.extendsRoactComponent)(state, classLikeAncestor);
    }
    return false;
}
function isNodeSymbolFromRobloxTypes(state, symbol) {
    var _a, _b;
    ...
    if (isNodeSymbolFromRobloxTypes(state, symbol)) {
        for (let i = 0; i < args.length; i++) {
            ...
            if (typescript_1.default.isCallExpression(nodeArg) && (0, types_1.isPossiblyType)(state.getType(nodeArg), types_1.isUndefinedType)) {
                args[i] = LuauAST_1.default.create(LuauAST_1.default.SyntaxKind.ParenthesizedExpression, {
                    expression: arg,
                });
            }
        }
...
function transformCallExpressionInner(state, node, expression, nodeArguments) {
    ...
    if (typescript_1.default.isSuperCall(node)) {
        if (isInsideRoactComponent(state, node)) {
            DiagnosticService_1.DiagnosticService.addDiagnostic(diagnostics_1.errors.missingSuperConstructorRoactComponent(node));
        }
        return LuauAST_1.default.call(LuauAST_1.default.property((0, convertToIndexableExpression_1.convertToIndexableExpression)(expression), "constructor"), [
    ...
    if (!LuauAST_1.default.list.isEmpty(prereqs) && (0, expressionMightMutate_1.expressionMightMutate)(state, expression, node.expression)) {
        expression = state.pushToVar(expression, "fn");
    }
    state.prereqList(prereqs);
    const exp = LuauAST_1.default.call((0, convertToIndexableExpression_1.convertToIndexableExpression)(expression), args);
...
function transformPropertyCallExpressionInner(state, node, expression, baseExpression, name, nodeArguments) {
    ...
    if (typescript_1.default.isSuperProperty(expression)) {
        ...
        return LuauAST_1.default.call(LuauAST_1.default.property((0, convertToIndexableExpression_1.convertToIndexableExpression)(baseExpression), expression.name.text), [
            LuauAST_1.default.globals.self,
            ...(0, ensureTransformOrder_1.ensureTransformOrder)(state, node.arguments),
        ]);
    }
    ...
    const [args, prereqs] = state.capture(() => (0, ensureTransformOrder_1.ensureTransformOrder)(state, nodeArguments));
    fixVoidArgumentsForRobloxFunctions(state, symbol, args, nodeArguments);
    if (!LuauAST_1.default.list.isEmpty(prereqs) && (0, expressionMightMutate_1.expressionMightMutate)(state, baseExpression, node.expression)) {
    ...
    if ((0, isMethod_1.isMethod)(state, expression)) {
        if (LuauAST_1.default.isValidIdentifier(name)) {
            ...
            exp = LuauAST_1.default.call(LuauAST_1.default.property((0, convertToIndexableExpression_1.convertToIndexableExpression)(baseExpression), name), args);
        }
    }
    else {
        exp = LuauAST_1.default.call(LuauAST_1.default.property((0, convertToIndexableExpression_1.convertToIndexableExpression)(baseExpression), name), args);
...
function transformElementCallExpressionInner(state, node, expression, baseExpression, argumentExpression, nodeArguments) {
    ...
    if (typescript_1.default.isSuperProperty(expression)) {
        ...
        }), [LuauAST_1.default.globals.self, ...(0, ensureTransformOrder_1.ensureTransformOrder)(state, node.arguments)]);
    }
    const expType = state.typeChecker.getNonOptionalType(state.getType(node.expression));
    const symbol = (0, types_1.getFirstDefinedSymbol)(state, expType);
    if (symbol) {
    ...
    const [args, prereqs] = state.capture(() => (0, ensureTransformOrder_1.ensureTransformOrder)(state, [argumentExpression, ...nodeArguments]));
