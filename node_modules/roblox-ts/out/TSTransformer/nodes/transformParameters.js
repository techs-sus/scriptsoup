...
function optimizeArraySpreadParameter(state, parameters, bindingPattern) {
    for (const element of bindingPattern.elements) {
        if (typescript_1.default.isOmittedExpression(element)) {
            LuauAST_1.default.list.push(parameters, LuauAST_1.default.tempId());
        }
        else {
...
function transformParameters(state, node) {
    ...
    for (const parameter of node.parameters) {
        ...
        if (parameter.initializer) {
            LuauAST_1.default.list.push(statements, (0, transformInitializer_1.transformInitializer)(state, paramId, parameter.initializer));
        }
        if (!typescript_1.default.isIdentifier(parameter.name)) {
            const bindingPattern = parameter.name;
            if (typescript_1.default.isArrayBindingPattern(bindingPattern)) {
                LuauAST_1.default.list.pushList(statements, state.capturePrereqs(() => (0, transformArrayBindingPattern_1.transformArrayBindingPattern)(state, bindingPattern, paramId)));
            }
            else {
                LuauAST_1.default.list.pushList(statements, state.capturePrereqs(() => (0, transformObjectBindingPattern_1.transformObjectBindingPattern)(state, bindingPattern, paramId)));
            }
        }
    }
    return {
