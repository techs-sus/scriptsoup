...
function diagnosticWithContext(category, contextFormatter, ...messages) {
    const result = (node, context) => {
        ...
        return (0, createDiagnosticWithLocation_1.createDiagnosticWithLocation)(result.id, messages.join("\n"), category, node);
    };
    result.id = id++;
    return result;
}
function diagnosticText(category, ...messages) {
    return (0, createTextDiagnostic_1.createTextDiagnostic)(messages.join("\n"), category);
}
...
exports.errors = {
    ...
    noLuaTupleInTemplateExpression: error("Can't use LuaTuple<T> in a template literal expression!", suggestion("Did you mean to add `[0]`?")),
