...
class RojoResolver {
    ...
    static findRojoConfigFilePath(projectPath) {
        ...
        if (candidates.length > 1) {
            (0, warn_1.warn)(`Multiple *.project.json files found, using ${candidates[0]}`);
        }
        return candidates[0];
    }
        ...
        if (fs_extra_1.default.pathExistsSync(realPath)) {
            ...
            try {
                ...
                if (isValidRojoConfig(configJson)) {
                    ...
                    (0, warn_1.warn)(`RojoResolver: Invalid configuration! ${ajv.errorsText(validateRojo.get().errors)}`);
                }
            }
        }
        else {
            (0, warn_1.warn)(`RojoResolver: Path does not exist "${rojoConfigFilePath}"`);
        }
    }
    ...
    getRbxPathFromFilePath(filePath) {
        ...
        for (const partition of this.partitions) {
            if ((0, isPathDescendantOf_1.isPathDescendantOf)(filePath, partition.fsPath)) {
        ...
        if (this.isGame) {
            if (rbxPath) {
                for (const container of from) {
                    if ((0, arrayStartsWith_1.arrayStartsWith)(rbxPath, container)) {
                        return container;
                    }
                }
            }
