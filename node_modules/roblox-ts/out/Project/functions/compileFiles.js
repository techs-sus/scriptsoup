...
function compileFiles(program, data, pathTranslator, sourceFiles) {
        ...
        (0, benchmark_1.benchmarkIfVerbose)(`running transformers..`, () => {
            ...
            if (transformers.length > 0) {
                if (!data.transformerWatcher) {
                    data.transformerWatcher = (0, createTransformerWatcher_1.createTransformerWatcher)(program);
                }
                ...
                for (const sourceFile of transformResult.transformed) {
                    if (typescript_1.default.isSourceFile(sourceFile)) {
                        updateFile(sourceFile.fileName, typescript_1.default.createPrinter().printFile(sourceFile));
                    }
                }
    ...
    for (let i = 0; i < sourceFiles.length; i++) {
        ...
        (0, benchmark_1.benchmarkIfVerbose)(`${progress} compile ${path_1.default.relative(process.cwd(), sourceFile.fileName)}`, () => {
            ...
            DiagnosticService_1.DiagnosticService.addDiagnostics((0, getCustomPreEmitDiagnostics_1.getCustomPreEmitDiagnostics)(sourceFile));
            if (DiagnosticService_1.DiagnosticService.hasErrors())
                return;
            ...
            if (DiagnosticService_1.DiagnosticService.hasErrors())
                return;
            const source = (0, LuauRenderer_1.renderAST)(luauAST);
